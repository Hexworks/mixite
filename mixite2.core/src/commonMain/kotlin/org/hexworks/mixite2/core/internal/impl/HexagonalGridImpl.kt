...
class HexagonalGridImpl<T : SatelliteData>(builder: HexagonalGridBuilder<T>) : HexagonalGrid<T> {
    ...
    private fun hexagon(coordinate: org.hexworks.mixite2.core.api.CubeCoordinate): HexagonImpl<T> {
        return HexagonImpl(gridData, coordinate, hexagonDataStorage)
    }

    override fun getHexagonsByCubeRange(from: org.hexworks.mixite2.core.api.CubeCoordinate, to: org.hexworks.mixite2.core.api.CubeCoordinate): Iterable<Hexagon<T>> {
        val coordinates = ArrayList<org.hexworks.mixite2.core.api.CubeCoordinate>(abs(from.gridZ-to.gridZ) + abs(from.gridX-to.gridX))

        for (gridZ in from.gridZ..to.gridZ) {
            for (gridX in from.gridX..to.gridX) {
                val coord = org.hexworks.mixite2.core.api.CubeCoordinate.fromCoordinates(gridX, gridZ)
    ...
    override fun getHexagonsByOffsetRange(gridXFrom: Int, gridXTo: Int, gridYFrom: Int, gridYTo: Int): Iterable<Hexagon<T>> {
        ...
        for (gridX in gridXFrom..gridXTo) {
            for (gridY in gridYFrom..gridYTo) {
                val cubeX = org.hexworks.mixite2.core.api.CoordinateConverter.convertOffsetCoordinatesToCubeX(gridX, gridY, gridData.orientation)
                val cubeZ = org.hexworks.mixite2.core.api.CoordinateConverter.convertOffsetCoordinatesToCubeZ(gridX, gridY, gridData.orientation)
                val coord = org.hexworks.mixite2.core.api.CubeCoordinate.fromCoordinates(cubeX, cubeZ)
                if (containsCubeCoordinate(coord)) {
                    coords.add(coord)
                }
            }
    ...
    override fun containsCubeCoordinate(coordinate: org.hexworks.mixite2.core.api.CubeCoordinate): Boolean {
        return this.hexagonDataStorage.containsCoordinate(coordinate)
    }

    private fun _getByCubeCoordinate(coordinate: org.hexworks.mixite2.core.api.CubeCoordinate) =
            if (containsCubeCoordinate(coordinate)) hexagon(coordinate) else null

    override fun getByCubeCoordinate(coordinate: org.hexworks.mixite2.core.api.CubeCoordinate): Maybe<Hexagon<T>> =
    ...
    override fun getByPixelCoordinate(coordinateX: Double, coordinateY: Double): Maybe<Hexagon<T>> {
        ...
        estimatedGridX = org.hexworks.mixite2.core.api.CoordinateConverter.convertOffsetCoordinatesToCubeX(estimatedGridX, estimatedGridZ, gridData.orientation)
        estimatedGridZ = org.hexworks.mixite2.core.api.CoordinateConverter.convertOffsetCoordinatesToCubeZ(estimatedGridX, estimatedGridZ, gridData.orientation)
        // it is possible that the estimated coordinates are off-grid so we
        // create a virtual hexagon
        val estimatedCoordinate = org.hexworks.mixite2.core.api.CubeCoordinate.fromCoordinates(estimatedGridX, estimatedGridZ)
    ...
    override fun getNeighborCoordinateByIndex(coordinate: org.hexworks.mixite2.core.api.CubeCoordinate, index: Int) =
            org.hexworks.mixite2.core.api.CubeCoordinate.fromCoordinates(
